...
/*
   ...
   */
public class _77_DoubleEleven {
    public static void main(String args[]) {
        Scanner in = new Scanner(System.in);
        String[] prices = in.nextLine().split(",");
        int sumPrice = Integer.parseInt(in.nextLine());
        in.close();
        int[] pricesInt = new int[prices.length];
        int i = 0;
        for (String str : prices) {
            pricesInt[i++] = Integer.parseInt(str);
        }
        Arrays.sort(pricesInt);
        int result = 0;
        for (int j = 0; j < pricesInt.length; j++) {
            if (result + pricesInt[j] > sumPrice) {
